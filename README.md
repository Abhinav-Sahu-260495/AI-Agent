...
yy- So, yeah, only the API url are going to change, right? 

aa- or nothing else change or how? Nothing else, nothing else.  So just change that URL into this one. Ensure that we're getting the same output in the same structure, nothing is broken, and let's let's push that in. And test it with multiple documents.  

2nd is, Vedaang was talking pyvegas. to see if we can use pie Vegas here. If we can, then, and that is easier, let me use pyvegas, but I need the API immediately, immediately. We have kept the user waiting for almost 2 weeks right now and we were only testing. did nothing on it.  

yy - Multimodel? Yeah. Okay, PyVegas, Vegas already internally using the pyvegas or something, right?  

aa - See, we can also use it. 

Abhinav - Anjali, uh, one thing. So, um, I think, uh, not only just URL will be changed, I think the payload will also be changed, right?  Because the payload format is different in this gateway too. Correct. Okay, 

Vedang - is this about the multimodel?  Yeah, yeah. Yeah. While doing the multimodel, can someone check the pi Vegas integration as well?   Yeah, because let's try to move away from the API integration part and directly start using the libraries.  
