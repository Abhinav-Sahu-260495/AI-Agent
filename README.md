...
So, common URL, uh, Vegas last app interface, we do chat, uh, slash up to this spot is a common. You can add it to dot ENV file. this spot and then this use case and context, you have to possess a dynamic. construct that you are, right? Okay.  Yeah, uh, that the payload uh, format, Gemini, native, and the, so if the user selecting that as a provider, what is that? Yeah, model ID. So we have to up and to here. a dynamic form.  So currently default is gym, right? So if they put it optional in the cloud, anthropic, and then open a, the user can choose if they want to multiple. Right?  Uh, I am not sure about this actually. Which one you're not sure about? The one which you said right now.  Oh, have you looking into that conversation, how it is working, that model name currently has a default. If it is a default, they are taking as a gymnic pro. Right?  If the user, if the sending is a specific model list, then whatever the user is sending, they will pick it up. Yeah. The same way user user is sending as if the plot, then you have to update as a provider, what are the payload you are constructing here is the all are mandatory.  Then you have to construct based on that one, right? These are the static, if the user is adulting as an anthropic or the clot, 3 points hours over. This provided this anthropic is a static on, you know.  So provider format also, somewhere, you have a configuration and just update with your payload and then you have to pass it, right? Okay, but then here, when you say that, like, you know, the payload will also get changed, right? Uh, the only difference was like here we are using the payload format as Vegas Unified.  That means that that particular format will be same and then later on, This will be changed by the API, right? This is what this API does. Now, uh, the payload will be different for different like native, Gemini, and the native clod.  So, what is a, this is a payload for the gym there, right? What are the line number 63 to, uh, 68, right? Uh-huh.  Yeah, so if the user, open the car, say, the user is, the model name is Gemini, then you have to construct this paler and you have to pause it. If it is a cloud, then you have to consume the cloud, payroll, and then pause the details. Okay.  Cool. I'll I'll look into that. How, how you are, yep.  You know everything you have, the metadata you are sending it, whatever, everything you are sending here. The same way they already are returned all the details in the AP. Just changing the cooking, the payload.  Okay. I look into that, Yogesh. Yeah, just the con, check the converse IP, how that, uh, from user, from our Python, right?  From there and then while calling the vegas, how the constructing the vegas paper. Okay. The Pegas payload, you have to change it.  That's it. Okay, okay, yeah. Okay, cool.  Okay, under the genie, right? You can get it there. I'm sorry?  No, no, the conversation under the genie folder. Yeah. You can see the...  You can have, like, large code actors, right? Yes, yes, yes. Yeah.  It's similar. Okay, I like it. Yeah, make the play just and the question to separate brands, cut it out and then push it to that.  And share that branch with me and tomorrow on my day. I can pull it down, you know, I can see them. Okay, I'll push the code.  I'll create a new branch, push the code, the whatever the changes is. Yeah. Not this sport.  So what are you going to make the changes, right? That's on the city. Yes, yes, yes.  I'll make a separate branch for that particular change, and then I'll push that into that same branch. You have a look into that. Oh, yeah. take the, yeah, upstream to the curious development, cut it out from the qverse development, uh, and then you can pursue a thing.
