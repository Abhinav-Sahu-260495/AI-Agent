...
So all 400s are handled in the same way. 409 is specifically for pre-existing values. I think we have to check that also. For every error, we can't customize the error loan.  No, no need to customize there are logs. We can generally say that other than 200, you can show the terror, right? No, but sometimes the error itself changes, right?  So for catalog, duplicate name, we have 409. And for some times, if it's a domain, you might not need to. You can populate that populate the terror message, dynamically, whatever the other message you're getting from APR.  The same one we're getting, correct. So yeah, this is odd quality. Yeah, the status quo, you can make it as a not 200.  Then the terror message, you can make it as a dynamic. The APNE AP returning that response, I think, and directly show on the user. Correct, makes sense.  Abu now, can you work on that so that we don't hardcore the error message to a certain status code? Okay, sure. Okay
